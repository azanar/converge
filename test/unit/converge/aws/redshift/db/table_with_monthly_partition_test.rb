...
class Converge::AWS::Redshift::DB::TableWithMonthlyPartitionTest < Test::Unit::TestCase
  include Converge::AWS::Redshift::DB::DBTestHelper
  ...
  test 'creating monthly partitions over a range' do
    ...
    tables = Converge::AWS::Redshift::DB::TableWithMonthlyPartition.from_date_range(@mock_model, @connection, {:min => start_time, :max => end_time})
  ...
  test 'loading date ranged table needing a new table' do
    ...
    table = Converge::AWS::Redshift::DB::TableWithMonthlyPartition.new(@mock_model, @connection, date)
